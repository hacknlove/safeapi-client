...
var onget = require('onget');
var jose = _interopDefault(require('node-jose'));
var fetchHelper = _interopDefault(require('@hacknlove/fetchhelper'));
...
async function createKey (data, alg) {
  ...
  if (uuid) {
    ...
    onget.set('dotted://safeapi.uuid', uuid);
  }
...
async function sFetch (url, options = {}) {
  if (!secret.pem) {
    await onget.waitUntil('dotted://safeapi.uuid');
  }
